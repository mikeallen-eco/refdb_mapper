harmonize_with_mol <- function(mol = mol_plants,
                      target = cites_plants,
                      manual_tax = cites_plants_manual_tax,
                      fuzzy_threshold = 0.97) {
  
  # --- Step 1 - exact matches between target full_sci_name and MOL Accepted

  message("Step 1 - exact matches between target full_sci_name and MOL Accepted ...")
  
  # exact scientific name match (including subspecies designation where it is included)
  # match_type 1: direct match between target$full_sci_name and MOL$Accepted
  exact_sci <- target %>%
    select(uid, full_sci_name) %>%
    left_join(
      mol %>% select(full_sci_name = Accepted) %>%
        mutate(match_type = "1 - exact_fullsci_w_ssp") %>%
        distinct(),
      by = join_by(full_sci_name)
    ) %>%
    mutate(mol_name = full_sci_name) %>%
    filter(match_type == "1 - exact_fullsci_w_ssp")

  message("   Matches found: ", nrow(exact_sci), " of ", nrow(target), " total\n")
  
  
  # --- Step 2 - exact matches between target full_sci_name and MOL Synonym
  
  message("Step 2 - exact matches between target full_sci_name and MOL Synonym ...")
  
  # exact synonym scientific name match (including target subspecies designations where given)
  # match_type 2: direct match between target$full_sci_name and MOL$Synonym
           
    exact_syn <- target %>%
    select(uid, full_sci_name) %>%
    filter(!uid %in% exact_sci$uid) %>%
    left_join(
      mol %>% select(full_sci_name = Synonym, mol_name = Accepted) %>%
  ...
  message(
    "   This excludes ",
    length(dup_uids),
    " uids matched to multiple MOL Accepted names via synonyms ..."
  )
  message("   ... these records will be matched manually.\n")
  }
  
  
  # --- Step 3 - exact matches between target genus_species and MOL Accepted (excluding ssp name)
  
  message("Step 3 - exact matches between target genus_species and MOL Accepted (excluding ssp name) ...")
  
  # exact scientific name match (regardless of target subspecies designation)
  # match_type 3: direct match between target$"Genus Species" and MOL$Accepted
  exact_sci_wo_ssp <- target %>%
    select(uid, genus_species, full_sci_name) %>%
    filter(!uid %in% exact_sci$uid, !uid %in% exact_syn$uid) %>%
    left_join(
      mol %>% select(genus_species = Accepted, mol_name = Accepted) %>%
  ...
  # --- Step 4 - exact matches between target genus_species and MOL Synonym (excluding ssp name)

  message("Step 4 - exact matches between target genus_species and MOL Synonym (excluding ssp name) ...")
  
  # exact synonym scientific name match (regardless of target subspecies designation)
  # match_type 4: direct match between target$genus_species and MOL$Synonym
  exact_syn_wo_ssp <- target %>%
    select(uid, genus_species, full_sci_name) %>%
    filter(!uid %in% exact_sci$uid,!uid %in% exact_syn$uid,!uid %in% exact_sci_wo_ssp$uid) %>%
    left_join(
      mol %>% select(genus_species = Synonym, mol_name = Accepted) %>%
  ...
  # --- Step 5 - FUZZY matches between target full_sci_name and MOL Accepted (> threshold)
  
  message("Step 5 - fuzzy matches between target full_sci_name and MOL Accepted ...")
  ...
  # --- Step 6 - FUZZY mmatches between target full_sci_name and MOL Synonym (> threshold)
  
  message("Step 6 - fuzzy matches between target full_sci_name and MOL Synonym ...")
  
  # FUZZY scientific name match (including subspecies designation where it is included)
  # match_type 6: fuzzy version of match_type 2 to correct misspellings
  
  candidates_syn <- mol$Synonym  
  ...
  # --- Step 7 - FUZZY matches between target genus_species and MOL Accepted (> threshold)

  message("Step 7 - fuzzy matches between target genus_species and MOL Accepted (excluding ssp name) ...")  
  
  # FUZZY scientific name match (regardless of target subspecies designations)
  ...
  if (is.null(manual_tax)) {
    ...
    manual <- data.frame(uid = NA, mol_name = NA, mol_name2 = NA)
  } 
  ...
  if (!is.null(manual_tax)) {
    ...
    message("   Found ", nrow(man %>% filter(!is.na(mol_name))), " non-blank manual matches ...")
    } else{
    ...
    manual <- target %>%
      select(uid, full_sci_name) %>%
      left_join(man %>% select(uid, mol_name, mol_name2), by = join_by(uid)) %>%
      mutate(match_type = case_when(uid %in% exact_sci$uid ~ "8.1 - manual - overriding type 1 match",
                                    ...
                                    uid %in% c(dup_uids, dup_uids2) ~ "8.8 - manual - resolved case with >1 MOL Accepted name",
                                    TRUE ~ "8.0 - manual - no automatic matches"))
    
    # remove blank rows
    manual <- manual %>% filter(!is.na(mol_name),
                                !mol_name %in% "")
  }
  ...
  final <- exact_sci %>%
    ...
    bind_rows(manual) %>%
    filter(!is.na(uid)) %>%
    right_join(target, by = join_by(uid, full_sci_name)) %>%
    mutate(dup = 1 * duplicated(uid)) %>%
    mutate(match_type = case_when(is.na(mol_name) & uid %in% c(dup_uids, dup_uids2) ~
                                    "9.1 - still unmatched, (>1 MOL Accepted)",
                                  is.na(mol_name) & !uid %in% c(dup_uids, dup_uids2) ~
                                    "9.2 - still unmatched (no automatic matches)",
                                  TRUE ~ match_type)) %>%
    arrange(match_type, uid) %>%
    mutate(fuzzy_score = jw_score) %>%
    select(-jw_score, -genus_species) %>%
    dplyr::rename(MOL_Accepted = mol_name, MOL_Accepted2 = mol_name2)
  ...
  message("   There are ",
          sum(is.na(final$MOL_Accepted)),
          " records still unmatched.\n"
  )
  message("   Edit the manual_tax file and iterate to manually assign and override the MOL Accepted names.")
    message(
      "   The manual_tax file should be tsv format with at least 3 columns:\n",
      "      - uid is the unique target id.\n",
      "      - mol_name is the MOL Accepted name assigned.\n",
      "      - mol_name2 is a second MOL Accepted name assigned if applicable (i.e., for splits)."
    )
  
    if (sum(is.na(final$MOL_Accepted)) > 0) {
